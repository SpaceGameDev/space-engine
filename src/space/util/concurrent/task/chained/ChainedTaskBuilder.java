...
public class ChainedTaskBuilder<FUNCTION> implements IChainedTaskBuilder<FUNCTION>, ToString, Cache {
	...
	public ChainedTaskBuilder(boolean singlethreadedOnly, boolean multithreadedOptimizeExecutionPriority, boolean multithreadedAllowTinyWorkload) {
	...
	protected synchronized ChainedTaskSinglethreaded<FUNCTION> getSinglethread() {
		...
		synchronized (cacheLock) {
			if (singlethread != null)
				return singlethread;
			return singlethread = new ChainedTaskSinglethreaded<>(ChainedTaskBuilder.this.list);
		}
	}
	...
	protected synchronized ChainedTaskMultithreaded<FUNCTION> getMultithread() {
		...
		synchronized (cacheLock) {
			if (multithread != null)
				return multithread;
			return multithread = new ChainedTaskMultithreaded<>(ChainedTaskBuilder.this.list, multithreadedOptimizeExecutionPriority, multithreadedAllowTinyWorkload);
		}
	}
			...
			public class NodeTaskMultithreaded extends AbstractRunnableTask implements ToString {
				...
				public void call() {
					if (callCnt.decrementAndGet() == 0) {
						if (allowTinyWorkload && node.func instanceof ITinyWorkload)
							this.run();
						else
