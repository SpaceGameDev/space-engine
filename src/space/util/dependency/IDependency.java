 ...
 * <br>
 * <h3>UUID Strings:</h3>
 * It is <b>strongly recommended</b> to add a uuid to all IDependency-Objects and put these UUIDs into public-static-final-Fields.
 ...
 * <ul>
 * <li>if the {@link IDependency} requires something (requires()-Method), something should be put in front of the {@link IDependency} object</li>
 * <li>if the {@link IDependency} is required by something (requiredBy()-Method), something should be put after the {@link IDependency} object</li>
 ...
 * @see NoDepDependency
 */
public interface IDependency extends Comparable<IDependency> {
	 ...
	 * comp > 1  ->  o2 before o1
	 */
	Comparator<IDependency> COMPARATOR = (o1, o2) -> {
	...
	static boolean find(IDependency func, String[] array) {
		if (array == null)
			return false;
		
		for (String str : array) {
			String uuid = func.uuid();
			if (uuid != null && ((IDependency.isPartialMatchingPattern(str) && uuid.contains(getPartialMatchingPattern(str))) || uuid.equals(str)))
				return true;
		}
		return false;
	}
	...
	@Override
	default int compareTo(IDependency o) {
