...
/**
 * An {@link ObservableCollection} allows you to be notified when something got added or removed form the {@link Collection}.
...
public class ObservableCollection<E> implements Collection<E> {
	...
	public ObservableCollection(Collection<E> delegate, EventEntry<Consumer<Change<E>>> listener) {
		this(delegate);
		this.changeEvent.addHook(listener);
	}
	
	public ObservableCollection(Collection<E> delegate, Consumer<Change<E>> listener) {
		this(delegate);
		this.changeEvent.addHook(listener);
	}
	...
	private @NotNull EventEntry<Consumer<Change<E>>> callAsStartedEmpty(@NotNull EventEntry<Consumer<Change<E>>> hook) {
		hook.function.accept(new Change<>() {
			...
			@Override
			public @NotNull ObservableCollection<E> collection() {
				return ObservableCollection.this;
			}
	...
	public Iterator<E> iterator() {
		return new Iterator<>() {
			...
			@Override
			public void remove() {
				ObservableCollection.this.remove(prev);
			}
		};
	}
	...
	public interface Change<E> {
		...
		@NotNull ObservableCollection<E> collection();
